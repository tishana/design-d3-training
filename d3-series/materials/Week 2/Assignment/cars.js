...
getUncommonMakes = (data) => {
    var umakes = 0
    data.forEach(d => {
        if (d.make_is_common == 0) {
            umakes += 1
        }
    });
    return umakes
...
getMakesPerCountry = (data) => {
    var counts = {}
    const mpc = data.map(x => { //getting a list of all of the countries
        counts[x.make_country] = (counts[x.make_country] || 0) + 1
    })
    return counts
}

// Source: https://stackoverflow.com/questions/43673734/how-to-count-number-of-occurrences-of-distinct-values-from-an-array-of-objects-i
